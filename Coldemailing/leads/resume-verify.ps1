...
# Spacing-Verletzung je Konto (aufeinanderfolgende Sends <600s)
$vio = DB "SELECT string_agg(email||' '||mg||'s', ', ') FROM (SELECT ea.email AS email, round(min(gap)) AS mg FROM (SELECT email_account_id, EXTRACT(EPOCH FROM (completed_at - lag(completed_at) OVER (PARTITION BY email_account_id ORDER BY completed_at))) AS gap FROM tasks WHERE task_type='campaign' AND status='completed' AND $dayFilter) g JOIN email_accounts ea ON ea.id=g.email_account_id WHERE gap IS NOT NULL AND gap < 600 GROUP BY ea.email) z"
...
if ((ToInt $aktiv) -lt 1)        { $probs += "keine aktive Kampagne (Resume nicht gelaufen?)" }
if ((ToInt $total) -eq 0)        { $probs += "0 Sends heute (Stall/Fenster?)" }
if ($vio)                        { $probs += "Spacing <600s: $vio" }
...
if ($probs.Count -gt 0) {
    $msg = "[Resume-Verify WARN] $uhr`n" + ($probs -join "`n") + "`nHeute $total Sends ($acct). Bitte pruefen."
} else {
    $msg = "[Resume-Verify OK] $uhr`nSauber getrickelt: $total Sends heute ($acct), alle Konto-Abstaende >=600s, kein :00-Cluster, STO aus, $aktiv Kampagnen aktiv. Naechster $next."
}
Log ($msg -replace "`n", ' | ')
Push $msg
